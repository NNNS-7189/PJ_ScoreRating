...
def score_othdiff():
    conf=messagebox.askquestion("確認", "楽曲名はそのままでスコアのみリセットされます\nリセットしますか？")
    if conf=="yes":
        txt1.delete(0, tk.END)
        txt2.delete(0, tk.END)
        txt3.delete(0, tk.END)
...
def main(): #旧getcalc
    ...
    try:
        with open("scores.csv", "r", encoding="utf-8") as f:
            ...
            if len(row) >= 3 and row[1] == title and row[2] == difficulty
            ]
    ...
    if old_best is None:
    # 初めての記録 → 何も出さない（好みでメッセージ出してもOK）
        pass

    elif new_rate > old_best:
        sa = new_rate - old_best
        lb_update.config(text=f"+{sa:.4f}% 更新しました！")

    else:
        sa = old_best - new_rate
        lb_update.config(text=f"ベストまであと{sa:.4f}%")



    
    record = [today, title, difficulty,new_rate, per, gre, goo, bad, mis]
    ...
    for row in rows:
        ...
        # 同じ曲 × 同じ難易度 の場合
        if row[1] == title and row[2] == difficulty:

            # 今回がベストなら置き換える
            if new_rate > float(row[3]):
...
def search_score():
    ...
    try:
        with open("scores.csv", "r", encoding="utf-8") as f:
            ...
            if len(best) >= 9:
                lb12.config(text=f"Perfect：{best[4]}")
                lb13.config(text=f"Great：{best[5]}")
                lb14.config(text=f"Good：{best[6]}")
                lb15.config(text=f"Bad：{best[7]}")
                lb16.config(text=f"Miss：{best[8]}")
            else:
                lb12.config(text="Perfect：記録なし")
                lb13.config(text="Great：記録なし")
                lb14.config(text="Good：記録なし")
                lb15.config(text="Bad：記録なし")
                lb16.config(text="Miss：記録なし")
...
bt2_1=tk.Button(text="同曲他難易度を記録",command=score_othdiff)
bt2_1.place(x=90,y=215)
...
lb6.place(x=30, y=250)

lb7 = tk.Label(text="達成率：")
lb7.place(x=30, y=270)

lb_update = tk.Label(text="")
lb_update.place(x=30, y=290)

fcap=tk.Label(text="")
fcap.place(x=30,y=310)
...
lb12=tk.Label(text="Perfect：")
lb12.place(x=270,y=160)

lb13=tk.Label(text="Great：")
lb13.place(x=270,y=180)

lb14=tk.Label(text="Good：")
lb14.place(x=270,y=200)

lb15=tk.Label(text="Bad：")
lb15.place(x=270,y=220)

lb16=tk.Label(text="Miss：")
lb16.place(x=270,y=240)

lb17=tk.Label(text="※楽曲名は完全一致でのみヒットするので打ち間違いにご注意ください",bg="#E4F7F7")
lb17.place(x=270, y=25)
